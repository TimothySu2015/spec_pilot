...
### ✨ addOperationIds
自動補充 OpenAPI 規格中缺少的 operationId ✨ NEW (Phase 9.5)
```
參數：
- specPath (必填): OpenAPI 規格檔案路徑（相對於專案根目錄）
- dryRun (選填): 預覽模式，不實際修改檔案（預設 false）
...
- 執行結果（成功/失敗）
- 總端點數統計
- 新增的 operationId 清單與詳細資訊
- 備份檔案路徑（成功時）
- dryRun 模式提示（預覽時）
```

**使用時機**：
- 當確認要自動修改規格檔案補充 operationId
- 先使用 dryRun 模式預覽變更，確認無誤後再實際修改
- 批次處理多個規格檔案

**特色功能**：
- 🔍 **DryRun 模式**：預覽變更不修改檔案，安全確認
- 💾 **自動備份**：修改前自動建立 `.bak` 備份檔案
- 📝 **保留格式**：使用 yaml.parseDocument() 保留原檔案格式和註解
- 🎯 **智慧命名**：符合 RESTful 慣例的 operationId 產生規則
- ✅ **選擇性修改**：只補充缺少的 operationId，已有的不修改
- 🛡️ **權限檢查**：自動檢查檔案可寫入性，避免權限錯誤
...
6. **自動補充 operationId（預覽模式）**：
   "請使用 addOperationIds 工具，設定 dryRun 為 true，預覽 specs/user-management-api.yaml 的變更"

7. **自動補充 operationId（實際修改）**：
   "確認預覽結果後，使用 addOperationIds 工具實際修改 specs/user-management-api.yaml"
